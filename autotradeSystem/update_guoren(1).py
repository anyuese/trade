...
def init():
    driver = webdriver.Chrome(r'C:\Program Files\Google\Chrome\Application\chromedriver.exe')
    dirname = r"F:\quantitative_trade"
    ...
    # 读取cookie
    with open(os.path.join(dirname, 'cookie.json'), 'r') as cookief:
...
def modify_2_years(driver, hrefs, year='两'):
    driver.implicitly_wait(40)
    num_month = '7'
    if year == '两':
        num_year = '2019'
    ...
    for k, v in hrefs.items():
        try:
            ...
            js_value = f'document.querySelector("#time-box > div.input-group.date.form_date_start > input").value="{num_year}/{num_month}/13"'  # js添加时间
            ...
            # driver.find_element_by_xpath(err_p).click()

            time.sleep(25)
        except Exception as e:
            pd.Series({k: v}).to_csv(f'error{year}.csv', mode='a', header=False)
            NOT_DONE_QUE[k] = [v]
            ic(e)
            ic('error')
            continue
...
if __name__ == '__main__':
    # write_cookie()
    driver = init()
    # parser_href(driver, path=5)
    # parser_href(driver, path=2)
    # pd.set_option('display.max_columns', None)
    #
    dict_ = dict()
    # dict1 = pd.read_csv('chaosuan_hrefs.csv')
    # dict2 = pd.read_csv('HT策略分类_hrefs.csv')
    ...
    modify_2_years(driver, dict_, year='三')
